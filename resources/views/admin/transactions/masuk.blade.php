...
<div class="p-6 space-y-5 bg-gray-50/50 dark:bg-gray-950 min-h-screen">
    ...
    {{-- 1. HEADER HALAMAN --}}
    <div class="rak-ticket p-6 bg-white dark:bg-[#111826] rounded-xl border border-gray-100 dark:border-gray-700/60 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4" style="border-left-color: rgba(20,184,166,0.5)">
        <div class="text-left">
            <h1 class="font-display text-xl font-bold text-gray-900 dark:text-white tracking-tight">Barang <span class="text-teal-500 dark:text-teal-400">Masuk</span></h1>
        ...
        <div class="shrink-0 flex items-center gap-2 w-full sm:w-auto">
            ...
            @if(strtolower(auth()->user()->role) === 'manajer gudang')
                <button type="button" data-modal-target="add-barang-masuk-modal" data-modal-toggle="add-barang-masuk-modal" class="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 bg-gradient-to-br from-[#1E293B] to-[#101826] text-amber-400 font-semibold rounded-xl text-sm px-4 py-2.5 shadow-md hover:shadow-lg transition-all duration-300">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="rak-ticket p-5 bg-white dark:bg-[#111826] border border-gray-100 dark:border-gray-700/60 rounded-xl shadow-sm flex items-center justify-between" style="border-left-color: rgba(20,184,166,0.5)">
            <span class="rak-tag absolute top-0 right-0 bg-teal-600 text-white text-[9px] font-semibold px-2 py-1 rounded-bl-lg">IN-ALL</span>
            <div class="space-y-1">
                <p class="rak-tag text-[10px] font-semibold text-gray-400 uppercase">Total Barang Masuk</p>
                <p class="font-display text-2xl font-bold text-teal-600 dark:text-teal-400 tracking-tight">{{ $transactions->sum('quantity') }} <span class="text-xs font-normal text-gray-400">Item</span></p>
            </div>
        ...
        <div class="rak-ticket p-5 bg-white dark:bg-[#111826] border border-gray-100 dark:border-gray-700/60 rounded-xl shadow-sm flex items-center justify-between" style="border-left-color: rgba(245,166,35,0.45)">
            <span class="rak-tag absolute top-0 right-0 bg-[#101826] text-amber-400 text-[9px] font-semibold px-2 py-1 rounded-bl-lg">IN-VAR</span>
            <div class="space-y-1">
                <p class="rak-tag text-[10px] font-semibold text-gray-400 uppercase">Variasi Produk Masuk</p>
    ...
    <div class="relative w-full">
        ...
        <input id="table-search" type="text" placeholder="Cari data barang masuk..." class="w-full pl-11 pr-4 py-3 text-xs font-medium rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-800 dark:text-white focus:ring-4 focus:ring-amber-400/20 focus:border-amber-400 transition-all shadow-sm placeholder:text-gray-400">
    </div>

    {{-- 4. KONTEN TABEL DATA TRANSAKSI --}}
    <div class="rak-ticket bg-white dark:bg-[#111826] rounded-xl border border-gray-100 dark:border-gray-700/60 shadow-sm overflow-hidden" style="border-left-color: rgba(20,184,166,0.5)">
        <span class="rak-tag absolute top-0 right-0 bg-teal-600 text-white text-[9px] font-semibold px-2.5 py-1 rounded-bl-lg z-10">IN-TBL</span>
            ...
            <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-700/60 text-left">
                ...
                <tbody id="transaction-table" class="divide-y divide-gray-100 dark:divide-gray-700/60 bg-white dark:bg-[#111826]">
                    ...
                    <tr class="hover:bg-amber-50/40 dark:hover:bg-gray-700/20 transition-colors duration-150">
                        ...
                        <td class="rak-tag px-6 py-5 whitespace-nowrap text-xs font-bold text-teal-600 dark:text-teal-400">
                        ...
                        <td class="px-6 py-5 whitespace-nowrap text-center">
                            <span class="rak-tag inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-teal-50 text-teal-700 border border-teal-100 text-[10px] font-bold uppercase dark:bg-teal-950/20 dark:text-teal-400 dark:border-teal-900/50">
                                <span class="material-symbols-outlined text-xs">arrow_downward</span> Masuk
                            </span>
                        </td>
                        <td class="font-display px-6 py-5 whitespace-nowrap text-right font-bold text-teal-600 dark:text-teal-400 text-sm">
                            +{{ $item->quantity }}
                        </td>
                        ...
                        <td class="px-6 py-5 whitespace-nowrap text-right pr-8">
                            <div class="flex items-center justify-end gap-1.5">
                                <button type="button" data-modal-target="detail-masuk-modal-{{ $item->id }}" data-modal-toggle="detail-masuk-modal-{{ $item->id }}" class="p-2 text-amber-600 hover:text-amber-700 bg-amber-50 hover:bg-amber-100/80 rounded-xl transition-colors border border-amber-100 dark:bg-amber-950/20 dark:text-amber-400 dark:border-amber-900/50" title="Lihat Detail">
                    ...
                    {{-- Modal Detail --}}
                    <div id="detail-masuk-modal-{{ $item->id }}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto h-[calc(100%-1rem)] max-h-full bg-gray-900/40 backdrop-blur-sm flex items-center justify-center">
                        <div class="rak-ticket relative w-full max-w-lg max-h-full bg-white rounded-xl shadow-xl dark:bg-[#111826] p-6 border border-gray-100 dark:border-gray-700/60 mx-auto mt-10 overflow-hidden" style="border-left-color: rgba(20,184,166,0.5)">
                            <span class="rak-tag absolute top-0 right-0 bg-teal-600 text-white text-[9px] font-semibold px-2.5 py-1 rounded-bl-lg">IN-DETAIL</span>
                            <div class="flex items-center justify-between border-b border-gray-100 dark:border-gray-700/60 pb-4 mb-5">
                                <h3 class="font-display text-base font-bold text-gray-900 dark:text-white flex items-center gap-2 tracking-tight">
                                    ...
                                    Detail Transaksi <span class="rak-tag text-teal-600 dark:text-teal-400 text-sm">#IN-{{ str_pad($item->id, 5, '0', STR_PAD_LEFT) }}</span>
                                </h3>
                            ...
                            <div class="space-y-4 text-left">
                                <div class="grid grid-cols-2 gap-4">
                                    <div><p class="rak-tag text-[10px] font-bold text-gray-400 uppercase mb-1">Nama Produk</p><p class="font-display text-sm font-bold text-gray-900 dark:text-white">{{ $item->product->name ?? 'Produk Terhapus' }}</p></div>
                                    <div><p class="rak-tag text-[10px] font-bold text-gray-400 uppercase mb-1">SKU</p><p class="rak-tag text-sm text-gray-700 dark:text-gray-300">{{ $item->product->sku ?? '-' }}</p></div>
                                    <div><p class="rak-tag text-[10px] font-bold text-gray-400 uppercase mb-1">Waktu Masuk</p><p class="text-sm font-semibold text-gray-900 dark:text-white">{{ \Carbon\Carbon::parse($item->date)->format('d M Y') }}</p></div>
                                    <div><p class="rak-tag text-[10px] font-bold text-gray-400 uppercase mb-1">Jumlah</p><p class="font-display text-sm font-bold text-teal-600 dark:text-teal-400">+{{ $item->quantity }} Pcs</p></div>
                                </div>
                                <div class="pt-2 border-t border-gray-100 dark:border-gray-700/60">
                                    <p class="rak-tag text-[10px] font-bold text-gray-400 uppercase mb-1.5">Keterangan / Asal Supplier</p>
                            ...
                            <div class="mt-6 pt-4 border-t border-gray-100 dark:border-gray-700/60 flex justify-end">
                                <button type="button" data-modal-toggle="detail-masuk-modal-{{ $item->id }}" class="px-4 py-2.5 text-xs font-semibold text-gray-500 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600">Tutup</button>
                            </div>
                        </div>
                    </div>
                    @empty
...
{{-- MODAL TAMBAH --}}
@if(strtolower(auth()->user()->role) === 'manajer gudang')
<div id="add-barang-masuk-modal" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto h-[calc(100%-1rem)] max-h-full bg-gray-900/40 backdrop-blur-sm flex items-center justify-center">
    <div class="rak-ticket relative w-full max-w-md bg-white rounded-xl shadow-xl dark:bg-[#111826] p-6 border border-gray-100 dark:border-gray-700/60 overflow-hidden" style="border-left-color: rgba(20,184,166,0.5)">
        <span class="rak-tag absolute top-0 right-0 bg-teal-600 text-white text-[9px] font-semibold px-2.5 py-1 rounded-bl-lg">NEW-IN</span>
        <h3 class="font-display text-base font-bold text-gray-900 dark:text-white mb-4 tracking-tight">Form Pasokan Barang Masuk</h3>
        <form action="{{ route('barang.masuk.store') }}" method="POST">
            @csrf
            <div class="space-y-4">
                <select name="product_id" required class="w-full rounded-xl border-gray-200 text-xs dark:bg-gray-800 dark:border-gray-600 py-2.5 focus:ring-4 focus:ring-amber-400/20 focus:border-amber-400">
                ...
                <input type="number" name="quantity" required placeholder="Jumlah" class="w-full rounded-xl border-gray-200 text-xs dark:bg-gray-800 dark:border-gray-600 py-2.5 focus:ring-4 focus:ring-amber-400/20 focus:border-amber-400">
                <textarea name="notes" rows="3" placeholder="Catatan" class="w-full rounded-xl border-gray-200 text-xs dark:bg-gray-800 dark:border-gray-600 py-2.5 focus:ring-4 focus:ring-amber-400/20 focus:border-amber-400"></textarea>
            </div>
            <div class="mt-6 flex justify-end gap-2">
                <button type="button" data-modal-toggle="add-barang-masuk-modal" class="px-4 py-2 text-xs font-semibold text-gray-500 bg-white border border-gray-200 rounded-xl dark:bg-gray-800 dark:border-gray-600 dark:text-gray-300">Batal</button>
                <button type="submit" class="px-4 py-2 text-xs font-semibold bg-gradient-to-br from-[#1E293B] to-[#101826] text-amber-400 rounded-xl shadow-md hover:shadow-lg transition-all duration-300">Kirim</button>
            </div>
        </form>
    </div>
</div>
