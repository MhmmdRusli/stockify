    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-gray-900 p-6 border border-gray-100 dark:border-gray-800 rounded-2xl shadow-sm">
        <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Kelola pengeluaran stok barang untuk distribusi toko, pesanan, atau mutasi eksternal.
            </p>
        </div>
        <div class="flex items-center gap-3 w-full sm:w-auto">
            ...
            {{-- TOMBOL AKTIF MEMBUKA MODAL --}}
            <button type="button" data-modal-target="add-barang-keluar-modal" data-modal-toggle="add-barang-keluar-modal" class="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-rose-600 hover:bg-rose-700 text-white font-semibold rounded-xl text-xs shadow-md shadow-rose-100 dark:shadow-none transition-all active:scale-95">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        <div class="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-5 rounded-2xl shadow-sm flex flex-col justify-between">
            <div class="flex items-center justify-between mb-4">
                <div class="p-2.5 bg-rose-50 dark:bg-rose-950/40 rounded-xl text-rose-600 dark:text-rose-400 flex items-center justify-center">
                    <span class="material-icons-outlined text-[22px]">call_made</span>
                </div>
                <span class="inline-flex items-center text-xs font-bold px-2.5 py-0.5 bg-rose-50 text-rose-700 dark:bg-rose-950/40 dark:text-rose-400 rounded-full border border-rose-100/40 dark:border-rose-900/30">-4%</span>
            </div>
            <div>
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400">Total Barang Keluar</p>
        ...
        <div class="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-5 rounded-2xl shadow-sm flex flex-col justify-between">
            <div class="flex items-center justify-between mb-4">
                <div class="p-2.5 bg-emerald-50 dark:bg-emerald-950/40 rounded-xl text-emerald-600 dark:text-emerald-400 flex items-center justify-center">
                    <span class="material-icons-outlined text-[22px]">storefront</span>
                </div>
                <span class="inline-flex items-center text-xs font-bold px-2.5 py-0.5 bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400 rounded-full border border-emerald-100/40 dark:border-emerald-900/30">Aktif</span>
            </div>
            <div>
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400">Total Variasi Produk Keluar</p>
    ...
    <div class="bg-white dark:bg-gray-900 p-4 border border-gray-100 dark:border-gray-800 rounded-2xl shadow-sm">
        <div class="flex flex-col sm:flex-row gap-3">
            <div class="flex-1 relative flex items-center">
                ...
                <input id="table-search" class="w-full pl-10 pr-4 py-2 bg-gray-50 dark:bg-gray-800 text-sm border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-rose-500 focus:border-rose-500 outline-none text-gray-900 dark:text-white placeholder-gray-400" placeholder="Cari data barang keluar..." type="text"/>
            </div>
        </div>
    </div>
        ...
        <div class="overflow-x-auto w-full">
            <table class="w-full text-left border-collapse">
                <thead class="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-100 dark:border-gray-800">
                    <tr>
                        <th class="px-6 py-4 text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">Waktu Keluar</th>
                        ...
                        <th class="px-6 py-4 text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">Keterangan / Tujuan</th>
                ...
                <tbody id="transaction-table" class="divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-gray-900">
                    ...
                    <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors group">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-xs font-bold text-rose-600 dark:text-rose-400 font-mono">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md bg-rose-50 text-rose-800 border border-rose-100/50 text-[10px] font-bold uppercase tracking-wider dark:bg-rose-950/30 dark:text-rose-400 dark:border-rose-900/30">
                                <span class="material-icons-outlined text-[12px]">arrow_upward</span> Keluar
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right font-bold text-rose-600 dark:text-rose-400 text-sm">
                            -{{ $item->quantity }}
                        </td>
...
{{-- MODAL POP-UP CONTAINER (FORM TAMBAH BARANG KELUAR) --}}
<div id="add-barang-keluar-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-white dark:bg-gray-900 rounded-2xl shadow border border-gray-100 dark:border-gray-800">
            <div class="flex items-center justify-between p-4 md:p-5 border-b border-gray-100 dark:border-gray-800 rounded-t">
                ...
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-950 dark:hover:bg-gray-800 dark:hover:text-white rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-toggle="add-barang-keluar-modal">
                    <span class="material-icons-outlined text-[20px]">close</span>
                </button>
            </div>
            <form action="{{ route('barang.keluar.store') }}" method="POST" class="p-4 md:p-5 space-y-4">
                @csrf
                <div>
                    ...
                    <select name="product_id" id="product_id" required class="bg-gray-50 border border-gray-200 dark:border-gray-700 text-gray-900 text-sm rounded-xl focus:ring-rose-500 focus:border-rose-500 block w-full p-2.5 dark:bg-gray-800 dark:text-white">
                ...
                <div>
                    <label for="quantity" class="block mb-2 text-xs font-bold uppercase tracking-wider text-gray-700 dark:text-gray-300">Jumlah Keluar</label>
                    <input type="number" name="quantity" id="quantity" min="1" required placeholder="Contoh: 15" class="bg-gray-50 border border-gray-200 dark:border-gray-700 text-gray-900 text-sm rounded-xl focus:ring-rose-500 focus:border-rose-500 block w-full p-2.5 dark:bg-gray-800 dark:text-white">
                </div>
                <div>
                    <label for="notes" class="block mb-2 text-xs font-bold uppercase tracking-wider text-gray-700 dark:text-gray-300">Keterangan / Tujuan Distribusi</label>
                    <textarea name="notes" id="notes" rows="3" placeholder="Contoh: Kirim ke Cabang Mall North Plaza atau retur vendor..." class="bg-gray-50 border border-gray-200 dark:border-gray-700 text-gray-900 text-sm rounded-xl focus:ring-rose-500 focus:border-rose-500 block w-full p-2.5 dark:bg-gray-800 dark:text-white"></textarea>
                </div>
                <button type="submit" class="w-full text-white bg-rose-600 hover:bg-rose-700 focus:ring-4 focus:outline-none focus:ring-rose-300 font-semibold rounded-xl text-sm px-5 py-2.5 text-center transition-all">
                    Kirim Pengajuan
                </button>
            </form>
        </div>
