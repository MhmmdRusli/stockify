    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="p-6 bg-white rounded-lg shadow-md dark:bg-gray-800 lg:col-span-1">
            <h2 class="text-xl font-semibold text-gray-800 dark:text-white mb-6 flex items-center gap-2">
                <span>📦</span> Pencatatan Barang Keluar
            </h2>
            ...
            <form action="{{ route('barang.keluar.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jumlah Kuantitas Keluar</label>
                    <input type="number" name="quantity" min="1" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required placeholder="Contoh: 10">
                </div>

                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Catatan / Keterangan</label>
                    <textarea name="notes" rows="3" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder="Contoh: Barang didistribusikan ke Toko Cabang B"></textarea>
                </div>

                <button type="submit" class="w-full text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-red-600 dark:hover:bg-red-700 focus:outline-none transition-colors">
        ...
        <div class="p-6 bg-white rounded-lg shadow-md dark:bg-gray-800 {{ Auth::user()->role === 'Manajer Gudang' ? 'lg:col-span-2' : 'lg:col-span-3' }}">
            <h3 class="text-xl font-semibold text-gray-800 dark:text-white mb-6 flex items-center gap-2">
                <span>📋</span> Riwayat Transaksi Keluar
            </h3>
                    ...
                    <tbody>
                        @forelse($transactions as $transaction)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                ...
                                <td class="px-4 py-4 text-red-600 font-bold">-{{ $transaction->quantity }}</td>
                                ...
                                <td class="px-4 py-4">
                                    @if($transaction->status === 'Pending')
                                        ...
                                        <div class="flex space-x-1.5 mt-1">
                                            <form action="{{ route('transactions.konfirmasi', $transaction->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menyetujui pengeluaran barang ini?');">
                                                @csrf
                                            ...
                                            <form action="{{ route('transactions.tolak', $transaction->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menolak pengeluaran barang ini?');">
                                                @csrf
                                    ...
                                    @elseif($transaction->status === 'Diterima' || $transaction->status === 'Dikeluarkan')
                                        <span class="bg-red-100 text-red-800 text-xs font-semibold px-2.5 py-1 rounded dark:bg-red-950 dark:text-red-300">{{ $transaction->status }}</span>
                                    @else
                        ...
                        @empty
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <td colspan="5" class="px-4 py-6 text-center text-gray-400 dark:text-gray-500">Belum ada riwayat barang keluar.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
