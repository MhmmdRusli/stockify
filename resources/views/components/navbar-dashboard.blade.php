        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center">
                ...
                <button type="button" data-dropdown-toggle="notification-dropdown"
                    class="relative p-2 text-gray-500 rounded-lg hover:text-gray-900 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-800">
                    ...
                    @if (!$showRealNotifications || $unreadCount > 0)
                        <span
                            class="absolute top-1.5 right-1.5 w-2 h-2 {{ $showRealNotifications ? 'bg-rose-500' : 'bg-amber-500' }} rounded-full ring-2 ring-white dark:ring-[#0C1220]"></span>
                    @endif
                </button>

                <div class="z-50 hidden max-w-sm my-4 overflow-hidden text-base list-none bg-white divide-y divide-gray-100 rounded-xl shadow-lg border border-gray-100 dark:divide-gray-700 dark:bg-[#111826] dark:border-gray-700"
                    id="notification-dropdown">
                    <div
                        class="rak-tag block px-4 py-2.5 text-[11px] font-semibold text-center text-amber-500 uppercase bg-gray-50 dark:bg-[#0C1220] tracking-wider">
                        Notifikasi
                    </div>
                            ...
                            @forelse($userNotifications as $notif)
                                ...
                                <a href="{{ $link }}?notification_id={{ $notif->id }}"
                                    ...
                                    <div class="flex-shrink-0">
                                        <span
                                            class="flex items-center justify-center w-10 h-10 rounded-full {{ $isOut ? 'bg-rose-50 text-rose-600 dark:bg-rose-900/30 dark:text-rose-400' : 'bg-teal-50 text-teal-600 dark:bg-teal-900/30 dark:text-teal-400' }}">
                                    ...
                                    <div class="w-full pl-3">
                                        <div class="text-gray-600 font-normal text-sm mb-1 dark:text-gray-300">
                                            {{ $notif->data['message'] ?? 'Notifikasi baru' }}</div>
                                        <div class="rak-tag text-[10px] font-medium text-amber-600 dark:text-amber-400">
                                            {{ $notif->created_at->diffForHumans() }}</div>
                                    </div>
                                </a>
                            @empty
                                <div class="px-4 py-6 text-center text-xs text-gray-400 dark:text-gray-500">Belum ada
                                    notifikasi baru.</div>
                            @endforelse
                        </div>
                    @else
                        ...
                        <div>
                            <a href="{{ url('/barang-masuk') }}"
                                class="flex px-4 py-3 border-b hover:bg-gray-50 dark:hover:bg-gray-800 dark:border-gray-700">
                                <div class="flex-shrink-0">
                                    <span
                                        class="flex items-center justify-center w-10 h-10 rounded-full bg-teal-50 text-teal-600 dark:bg-teal-900/30 dark:text-teal-400">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path>
                                ...
                                <div class="w-full pl-3">
                                    <div class="text-gray-600 font-normal text-sm mb-1 dark:text-gray-300">Barang masuk
                                        baru menunggu <span
                                            class="font-semibold text-gray-900 dark:text-white">konfirmasi</span></div>
                                    <div class="rak-tag text-[10px] font-medium text-amber-600 dark:text-amber-400">
                                        beberapa saat lalu</div>
                                </div>
                            </a>
                            <a href="{{ url('/products') }}"
                                class="flex px-4 py-3 border-b hover:bg-gray-50 dark:hover:bg-gray-800 dark:border-gray-700">
                                <div class="flex-shrink-0">
                                    <span
                                        class="flex items-center justify-center w-10 h-10 rounded-full bg-rose-50 text-rose-600 dark:bg-rose-900/30 dark:text-rose-400">
                                ...
                                <div class="w-full pl-3">
                                    <div class="text-gray-600 font-normal text-sm mb-1 dark:text-gray-300">Stok <span
                                            class="font-semibold text-gray-900 dark:text-white">3 produk</span> berada
                                        di bawah ambang minimum</div>
                                    <div class="rak-tag text-[10px] font-medium text-amber-600 dark:text-amber-400">10
                                        menit lalu</div>
                                </div>
                            </a>
                        </div>
                    ...
                    <a href="{{ url('/report/transactions') }}"
                        class="block py-2.5 rak-tag text-sm font-medium text-center text-gray-700 bg-gray-50 hover:bg-gray-100 dark:bg-[#0C1220] dark:text-gray-300 dark:hover:bg-gray-800">
